...
At that time patch antennas were being mostly used in spaceborne applications~\cite{balanis2016antenna}, while in current times the \glspl{MPA} are still used for aerospace applications and low-profile communications~\cite{garg2001microstrip}. 
...
To properly find an answer to the proposed question, some objectives must be defined.:
\begin{itemize}
    \item Design of a multiband \Glsfmtshort{MPA};
    \item Simulation of the designed antenna using MATLAB\textsuperscript{TM}/CST;
    \item Antenna optimization;
    \item Fabrication and measurement of the antenna's performance.
\end{itemize}

\section{Document Structure}
This document is devided into four chapters. Starting with this, which is~\ref{cha:introduction}, is where the context, problem statement and objectives are presented.
In~\ref{cha:stateoftheart} a literature review on \glspl{MPA} is presented, where its fundamentals are explained as well as the implementation techniques for multiband \glspl{MPA} are shown.
Next in~\ref{cha:workplan} is presented this thesis' work plan, a breakdown of the tasks and respective timeline as well as work done and what is expected to be delivered.
Finally,~\ref{cha:conclusions} presents some conclusions of the work done and some future work to be made.